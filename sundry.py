...
def prt(str, level=0, warning_level=0):
    ...
    if rpl == 'no':
        ...
        if level == 0:
            ...
            print(
                f'|{warning_str:<4} Re:{time:<20} {indent_str:<70}{warning_str:>4}|')
...
def prt_log(str, level, warning_level):
    ...
    if rpl == 'yes':
        format_width = 105
        db = consts.glo_db()
        oprt_id = db.get_oprt_id_via_db_id(
            consts.glo_tsc_id(), consts.glo_log_id())
...
def handle_exception(str='', level=0, warning_level=0):
    ...
    if rpl == 'yes':
        ...
        if exception_info:
            prt('The transaction was interrupted because of an exception',
                1, warning_level=2)
            ...
            oprt_id = db.get_oprt_id_via_db_id(
                consts.glo_tsc_id(), consts.glo_log_id())
            prt(
                f'Unable to get data from the database.oprt_id:{oprt_id}', 3, 2)
            raise consts.ReplayExit
