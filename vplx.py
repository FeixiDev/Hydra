...
def get_disk_dev():
    ...
    if disk_dev:
        s.pwl(
            f'Succeed in getting disk device {disk_dev} with id {consts.glo_id()}', 3, '', 'finish')
        return disk_dev
    else:
        s.scsi_rescan(SSH, 'a')
        s.pwl(
            f'No disk with SCSI ID {consts.glo_id()} found, scan again...', 3, '', 'start')
        disk_dev = _find_new_disk()
        if disk_dev:
...
class VplxDrbd(object):
    ...
    def _create_iscsi_session(self):
        ...
        if not s.find_session(NETAPP_IP, SSH):
            s.pwl(
                f'No session found, start to login to {NETAPP_IP}', 3, '', 'start')
    ...
    def prepare_config_file(self):
        ...
        s.pwl(
            f'Succeed in creating DRBD config file "{self.res_name}.res"', 3, '', 'finish')
    ...
    def _drbd_init(self):
        ...
        if init_result:
            ...
            if re_result:
                s.pwl(
                    f'Succeed in initializing DRBD resource "{self.res_name}"', 4, oprt_id, 'finish')
                return True
            else:
    ...
    def _drbd_up(self):
        ...
        s.pwl(
            f'Start to bring up DRBD resource "{self.res_name}"', 3, oprt_id, 'start')
        result = s.get_ssh_cmd(SSH, unique_str, cmd, oprt_id)
        if result:
            if result['sts']:
                s.pwl(
                    f'Succeed in bringing up DRBD resource "{self.res_name}"', 4, oprt_id, 'finish')
                return True
            else:
    ...
    def _drbd_primary(self):
        '''
        Complete initial synfchronization of resources
        '''
        oprt_id = s.get_oprt_id()
        unique_str = '7C4LU6Xr'
        cmd = f'drbdadm primary --force {self.res_name}'
        s.pwl(
            f'Start to initial synchronization for "{self.res_name}"', 3, oprt_id, 'start')
        result = s.get_ssh_cmd(SSH, unique_str, cmd, oprt_id)
        if result:
            if result['sts']:
                s.pwl(
                    f'Succeed in synchronizing DRBD resource "{self.res_name}"', 4, oprt_id, 'finish')
                return True
            else:
    ...
    def drbd_status_verify(self):
        ...
        s.pwl(
            f'Start to check DRBD resource "{self.res_name}" status', 3, oprt_id, 'start')
            ...
            if result['sts']:
                ...
                if re_result:
                    ...
                    if status == 'UpToDate':
                        ...
                        s.pwl(
                            f'Succeed in checking DRBD resource "{self.res_name}"', 4, oprt_id, 'finish')
    ...
    def _drbd_del_config(self, res_name):
        ...
        if del_result['sts']:
            print(
                f'  Removed the DRBD resource {res_name} config file successfully')
            return True
        else:
...
class VplxCrm(object):
    ...
    def _crm_create(self):
        ...
        unique_str = 'LXYV7dft'
        s.pwl(
            f'Start to create iSCSILogicalUnit resource "{self.lu_name}"', 3, oprt_id, 'start')
        ...
        if result:
            if result['sts']:
                s.pwl(
                    f'Succeed in creating iSCSILogicaLUnit "{self.lu_name}"', 4, oprt_id, 'finish')
                return True
            else:
                s.pwce(
                    f'Failed to create iSCSILogicaLUnit "{self.lu_name}"', 4, 2)
        else:
            s.handle_exception()
    ...
    def _setting_col(self):
        ...
        s.pwl(
            f'Start to set up colocation of iSCSILogicalUnit "{self.lu_name}"', 3, oprt_id, 'start')
        result_crm = s.get_ssh_cmd(SSH, unique_str, cmd, oprt_id)
        if result_crm:
            if result_crm['sts']:
                s.pwl(
                    f'Succeed in set colocation of "{self.lu_name}"', 4, oprt_id, 'finish')
                return True
            else:
    ...
    def _setting_order(self):
        ...
        s.pwl(
            f'Start to set up order of iSCSILogicalUnit "{self.lu_name}"', 3, oprt_id, 'start')
        result_crm = s.get_ssh_cmd(SSH, unique_str, cmd, oprt_id)
        if result_crm:
            if result_crm['sts']:
                s.pwl(
                    f'Succeed in set order of "{self.lu_name}"', 4, oprt_id, 'finish')
                return True
            else:
    ...
    def _crm_start(self):
        ...
        s.pwl(
            f'Start up the iSCSILogicalUnit resource "{self.lu_name}"', 3, oprt_id, 'start')
        result_cmd = s.get_ssh_cmd(SSH, unique_str, cmd, oprt_id)
        if result_cmd:
            if result_cmd['sts']:
                s.pwl(
                    f'Succeed in starting up iSCSILogicaLUnit "{self.lu_name}"', 4, oprt_id, 'finish')
                return True
            else:
                s.pwce(
                    f'Failed to start up iSCSILogicaLUnit "{self.lu_name}"', 4, 2)
        else:
            s.handle_exception()
    ...
    def _crm_stop(self, res_name):
        ...
        if crm_stop['sts']:
            if self.cyclic_check_crm_status(res_name, 'Stopped'):
                print(
                    f'  Stopped the iSCSILogicalUnit resource {res_name} successfully')
                return True
            else:
    ...
    def _crm_del(self, res_name):
        ...
        if del_result:
            ...
            if len(re_result) == 2:
                print(
                    f'  Removed the iSCSILogicalUnit resource {res_name} successfully')
                return True
            else:
