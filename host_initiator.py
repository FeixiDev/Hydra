...
def get_disk_dev():
    ...
    if disk_dev:
        ...
        s.pwl(
            f'No disk with SCSI ID {scsi_id} found, scan again...', 3, '', 'start')
        s.scsi_rescan(SSH, 'a')
        disk_dev = _find_new_disk()
        if disk_dev:
...
class HostTest(object):
    ...
    def _create_iscsi_session(self):
        ...
        if not s.find_session(VPLX_IP, SSH):
            s.pwl(
                f'No session found, start to login to {VPLX_IP}', 3, '', 'start')
    ...
    def _mount(self, dev_name):
        ...
        s.pwl(
            f'Start trying to mount {dev_name} to {MOUNT_POINT}', 2, oprt_id, 'start')
        result_mount = s.get_ssh_cmd(SSH, unique_str, cmd, oprt_id)
        if result_mount:
            if result_mount['sts']:
                s.pwl(f'Disk {dev_name} mounted to {MOUNT_POINT}',
                      3, oprt_id, 'finish')
                return True
            else:
